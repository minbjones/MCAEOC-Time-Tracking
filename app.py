...
def ensure_role_catalog(conn):
    ...
    cursor.execute(
        """
        IF NOT EXISTS (SELECT 1 FROM dbo.Roles WHERE RoleName = 'Leave Manager')
            INSERT INTO dbo.Roles (RoleName) VALUES ('Leave Manager');
        """
    )
    conn.commit()
...
def ensure_timesheet_admin_schema(conn):
    ...
    cursor.execute(
        """
        IF COL_LENGTH('dbo.TimeEntries', 'Notes') IS NULL
            ALTER TABLE dbo.TimeEntries ADD Notes NVARCHAR(500) NULL;

        IF COL_LENGTH('dbo.TimeEntries', 'EntrySource') IS NULL
            ALTER TABLE dbo.TimeEntries ADD EntrySource NVARCHAR(20) NULL;

        IF COL_LENGTH('dbo.TimeEntries', 'CreatedByEmployeeId') IS NULL
            ALTER TABLE dbo.TimeEntries ADD CreatedByEmployeeId INT NULL;

        IF COL_LENGTH('dbo.TimeEntries', 'LastModifiedAt') IS NULL
            ALTER TABLE dbo.TimeEntries ADD LastModifiedAt DATETIME2 NULL;

        IF COL_LENGTH('dbo.TimeEntries', 'LastModifiedByEmployeeId') IS NULL
            ALTER TABLE dbo.TimeEntries ADD LastModifiedByEmployeeId INT NULL;

        IF NOT EXISTS
        (
            SELECT 1
            FROM sys.default_constraints
            WHERE parent_object_id = OBJECT_ID('dbo.TimeEntries')
              AND name = 'DF_TimeEntries_EntrySource'
        )
            ALTER TABLE dbo.TimeEntries
            ADD CONSTRAINT DF_TimeEntries_EntrySource DEFAULT ('Clock') FOR EntrySource;

        UPDATE dbo.TimeEntries
        SET EntrySource = 'Clock'
        ...
        (
            ...
            WHERE name = 'FK_TimeEntries_CreatedByEmployee'
        )
            ALTER TABLE dbo.TimeEntries
            ADD CONSTRAINT FK_TimeEntries_CreatedByEmployee
        ...
        (
            SELECT 1
            FROM sys.foreign_keys
            WHERE name = 'FK_TimeEntries_LastModifiedByEmployee'
        )
            ALTER TABLE dbo.TimeEntries
            ADD CONSTRAINT FK_TimeEntries_LastModifiedByEmployee
            FOREIGN KEY (LastModifiedByEmployeeId) REFERENCES dbo.Employees(EmployeeId);

        IF OBJECT_ID('dbo.TimeEntryAudit', 'U') IS NULL
        BEGIN
        ...
        if viewer_role != "Owner":
            cursor.execute(
                ...
                WHERE LeaveRequestId = ?
                  AND PayrollId IN (SELECT PayrollId FROM EmployeeTree)
                """,
                current_user_id(),
                leave_request_id,
            )
    ...
    with get_connection() as conn:
        ...
        if viewer_role == "Owner":
            cursor.execute(
                ...
                (
                    SELECT
                        T.PayrollId AS EmployeeId,
                        T.TimeEntryId,
                        T.ClockInTime,
                        T.ClockOutTime,
                        ROW_NUMBER() OVER (PARTITION BY T.PayrollId ORDER BY ISNULL(T.ClockOutTime, T.ClockInTime) DESC, T.TimeEntryId DESC) AS RowNum
                    FROM dbo.TimeEntries T
                )
                SELECT
                ...
                FROM dbo.Employees E
                INNER JOIN dbo.Roles R ON R.RoleId = E.RoleId
                LEFT JOIN LatestEntry L ON L.EmployeeId = E.PayrollId AND L.RowNum = 1
                WHERE E.IsActive = 'Yes'
                ORDER BY E.LastName, E.FirstName
                """
            )
            ...
            cursor.execute(
                ...
                INNER JOIN dbo.Roles R ON R.RoleId = E.RoleId
                LEFT JOIN LatestEntry L ON L.EmployeeId = E.PayrollId AND L.RowNum = 1
                WHERE E.PayrollId IN (SELECT PayrollId FROM EmployeeTree)
                ORDER BY E.LastName, E.FirstName
                """,
                current_user_id(),
            )
...
def create_manual_time_entry(employee_id: int):
    ...
    with get_connection() as conn:
        ...
        cursor.execute(
            """
            SELECT PayrollId AS EmployeeId,
                   LTRIM(RTRIM(CONCAT(FirstName, ' ', LastName))) AS FullName
            FROM dbo.Employees
            WHERE PayrollId = ?
            """,
        ...
        if request.method == "POST":
            ...
            cursor.execute(
                ...
                OUTPUT INSERTED.TimeEntryId
                VALUES
                (
                    ?, ?, ?, ?, 'Manual', ?, SYSUTCDATETIME(), ?
                )
                """,
                employee.EmployeeId,
                clock_in_time,
